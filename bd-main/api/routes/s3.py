...
async def s3_settings_get(action: Optional[str] = Query(None)):
    with get_cursor() as cur:
        ...
        return {
            "settings": {
                "bucket_name": s[0],
                "endpoint_url": s[1] or YANDEX_ENDPOINT,
                "access_key": s[2],
                "secret_key_masked": mask_key(s[3]),
...
async def s3_settings_update(data: dict):
    with get_cursor() as cur:
        ...
        endpoint = (
            data.get("endpoint_url", s[1] if s else YANDEX_ENDPOINT)
            or YANDEX_ENDPOINT
        )
        ...
        if s:
            cur.execute(
                ...
                SET bucket_name=%s, endpoint_url=%s, access_key=%s,
                    secret_key=%s, updated_at=NOW()
                WHERE id=1
                """,
            ...
            cur.execute(
                f"""
                INSERT INTO {SCHEMA}.s3_settings
                    (id, bucket_name, endpoint_url, access_key, secret_key)
                VALUES (1, %s, %s, %s, %s)
                """,
                (bucket, endpoint, access, secret),
            )
        ...
        return {
            ...
            "settings": {
                ...
                "access_key": access,
                "secret_key_masked": mask_key(secret),
                "configured": configured,
            },
        }