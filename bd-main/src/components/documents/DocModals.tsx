...
interface TxModalProps {
  ...
  txForm: { description: string; amount: string; date: string; category: string; is_cashless: boolean };
  txSaving: boolean;
  txSaved: boolean;
  ...
  onClose: () => void;
  onSave: () => void;
  onFormChange: (patch: Partial<{ description: string; amount: string; date: string; category: string; is_cashless: boolean }>) => void;
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <div className="w-full sm:max-w-md card-fin rounded-t-2xl sm:rounded-xl p-5 space-y-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {/* is_cashless toggle */}
        <button type="button" onClick={() => onFormChange({ is_cashless: !txForm.is_cashless })}
          className={`w-full flex items-center gap-3 p-3 rounded-lg border transition-all text-left ${txForm.is_cashless ? "border-blue-400/40 bg-blue-900/10" : "border-border"}`}>
          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${txForm.is_cashless ? "border-blue-400 bg-blue-500" : "border-border"}`}>
            {txForm.is_cashless && <Icon name="Check" size={12} className="text-primary-foreground" />}
          </div>
          <div>
            <div className="text-sm font-medium">Безналичный расчёт</div>
            <div className="text-xs text-muted-foreground">Отметить, что оплата прошла по безналу</div>
          </div>
        </button>
...
// ── No-photo document creation modal ─────────────────────────
interface NoPhotoModalProps {
  show: boolean;
  form: { name: string; amount: string; date: string; category: string; description: string; is_cashless: boolean };
  saving: boolean;
  customCategories: string[];
  showNewCat: boolean;
  newCatInput: string;
  onClose: () => void;
  onSave: () => void;
  onFormChange: (patch: Partial<NoPhotoModalProps["form"]>) => void;
  onShowNewCat: (v: boolean) => void;
  onNewCatInput: (v: string) => void;
  onSaveCustomCategory: (name: string) => void;
  onSetCustomCategories: (cats: string[]) => void;
...
export function NoPhotoModal({
  show, form, saving, customCategories, showNewCat, newCatInput,
  onClose, onSave, onFormChange, onShowNewCat, onNewCatInput,
  onSaveCustomCategory, onSetCustomCategories,
}: NoPhotoModalProps) {
  if (!show) return null;
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center pt-4 sm:pt-10 p-4 overflow-y-auto" onClick={onClose}>
      <div className="w-full max-w-lg card-fin rounded-xl p-4 sm:p-5 space-y-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold">Добавить документ без фото</h2>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground"><Icon name="X" size={18} /></button>
        </div>

        <div className="space-y-3">
          <div>
            <label className="text-xs text-muted-foreground block mb-1">Название документа *</label>
            <input value={form.name} onChange={(e) => onFormChange({ name: e.target.value })}
              placeholder="Например: Акт сверки №123"
              className="w-full bg-secondary border border-border rounded px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-gold" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
            ...
            <div>
              <label className="text-xs text-muted-foreground block mb-1">Дата *</label>
              <input type="date" value={form.date} onChange={(e) => onFormChange({ date: e.target.value })}
                className="w-full bg-secondary border border-border rounded px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-gold" />
            </div>
          </div>
          <div>
            <label className="text-xs text-muted-foreground block mb-1">Статья затрат</label>
            {showNewCat ? (
              <div className="flex gap-1">
          ...
          <div>
            <label className="text-xs text-muted-foreground block mb-1">Описание / Комментарий</label>
            <textarea value={form.description} onChange={(e) => onFormChange({ description: e.target.value })}
              placeholder="Дополнительная информация..."
              rows={3}
              className="w-full bg-secondary border border-border rounded px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-gold resize-none" />
          </div>
        </div>

        {/* is_cashless toggle */}
        <button type="button" onClick={() => onFormChange({ is_cashless: !form.is_cashless })}
          className={`w-full flex items-center gap-3 p-3 rounded-lg border transition-all text-left ${form.is_cashless ? "border-blue-400/40 bg-blue-900/10" : "border-border"}`}>
          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${form.is_cashless ? "border-blue-400 bg-blue-500" : "border-border"}`}>
            {form.is_cashless && <Icon name="Check" size={12} className="text-primary-foreground" />}
          </div>
          <div>
            <div className="text-sm font-medium">Безналичный расчёт</div>
            <div className="text-xs text-muted-foreground">Отметить, что оплата прошла по безналу</div>
        ...
        <button onClick={onSave} disabled={saving || !form.name || !form.amount || !form.date}
