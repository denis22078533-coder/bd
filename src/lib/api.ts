...
const URLS = {
  imgProxy: `${BASE}/img-proxy`,
  transactions: `${BASE}/transactions`,
  documents: `${BASE}/documents`,
  taxReports: `${BASE}/tax-reports`,
  aiSettings: `${BASE}/ai-settings`,
  aiChat: `${BASE}/ai-chat`,
  recognizeDoc: `${BASE}/recognize-doc`,
  s3Settings: `${BASE}/s3-settings`,
  uploadDoc: `${BASE}/upload-doc`,
  generatePdf: `${BASE}/generate-pdf`,
  docsPdf: `${BASE}/docs-pdf`,
};

// ── S3 config cache for direct image URLs ──────────────────
let _s3Config: { bucket_name: string; endpoint_url: string } | null = null;

export function setS3Config(config: { bucket_name: string; endpoint_url: string } | null) {
  _s3Config = config;
  try {
    if (config) {
      localStorage.setItem("s3_config_cache", JSON.stringify(config));
    } else {
...
function getS3Config(): { bucket_name: string; endpoint_url: string } | null {
  if (_s3Config) return _s3Config;
  try {
    const cached = localStorage.getItem("s3_config_cache");
    if (cached) {
      _s3Config = JSON.parse(cached);
      return _s3Config;
    }
  } catch { /* ignore */ }
  return null;
...
/** Строит прямой URL к файлу в S3 или использует прокси как fallback */
export function proxyImg(url: string | null | undefined): string | undefined {
  if (!url) return undefined;
  // Если URL уже содержит полный путь (не относительный) — используем как есть
  if (url.startsWith("http://") || url.startsWith("https://")) {
    // Если это Яндекс S3 и у нас есть настройки бакета — строим прямой URL
    if (url.includes("storage.yandexcloud.net")) {
      const cfg = getS3Config();
      if (cfg?.bucket_name && cfg?.endpoint_url) {
        // Извлекаем относительный путь: storage.yandexcloud.net/bucket_name/path → /path
        const match = url.match(/\/[^/]+\/[^/]+\/(.+)/);
        if (match) {
          const relativePath = match[1];
          return `${cfg.endpoint_url.replace(/\/+$/, "")}/${cfg.bucket_name}/${relativePath}`;
        }
      }
  ...
  // Если это относительный путь — строим URL из настроек
  const cfg = getS3Config();
  if (cfg?.bucket_name && cfg?.endpoint_url) {
    return `${cfg.endpoint_url.replace(/\/+$/, "")}/${cfg.bucket_name}/${url.replace(/^\//, "")}`;
  }
  return url;
}
...
export interface DashboardSummary {
  ...
  expense_year: number;
  profit_month: number;
  cashless_month: number;
  cashless_year: number;
...
export const api = {
  ...
  fixS3Acl: (): Promise<{ ok: boolean; fixed: number; errors_count: number; errors: { id: number; key: string; error: string }[] }> =>
    request<{ ok: boolean; fixed: number; errors_count: number; errors: { id: number; key: string; error: string }[] }>(`${BASE}/fix-s3-acl`, { method: "POST" }),

  // ─── Categories (статьи затрат) ─────────────────────────
  categories: {
    list: () => request<{ categories: { name: string; is_default: boolean }[] }>(`${BASE}/categories`),
    add: (name: string) => request<{ ok: boolean; name: string }>(`${BASE}/categories`, { method: "POST", body: JSON.stringify({ name }) }),
    remove: (name: string) => request<{ ok: boolean }>(`${BASE}/categories?name=${encodeURIComponent(name)}`, { method: "DELETE" }),
  },
