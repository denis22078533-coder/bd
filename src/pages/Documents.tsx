...
import { MergeDialog, DeleteDialog, TxModal, MultiModal, NoPhotoModal } from "@/components/documents/DocModals";
...
export default function Documents() {
  ...
  const [txForm, setTxForm] = useState({ description: "", amount: "", date: "", category: "Прочее", is_cashless: false });
  ...
  // ── No-photo document state ──────────────────────────────
  const [showNoPhotoModal, setShowNoPhotoModal] = useState(false);
  const [noPhotoForm, setNoPhotoForm] = useState({ name: "", amount: "", date: new Date().toISOString().split("T")[0], category: "Прочее", description: "", is_cashless: false });
  const [noPhotoSaving, setNoPhotoSaving] = useState(false);

  const handleCreateNoPhoto = async () => {
    if (!noPhotoForm.name || !noPhotoForm.amount || !noPhotoForm.date) return;
    setNoPhotoSaving(true);
    try {
      // 1. Создаём документ со статусом "done" (без фото)
      const docRes = await api.documents.create({
        name: noPhotoForm.name,
        status: "done",
        rec_amount: `₽ ${Number(noPhotoForm.amount).toLocaleString("ru-RU")}`,
        rec_date: noPhotoForm.date,
        rec_type: noPhotoForm.description || noPhotoForm.name,
        rec_category: noPhotoForm.category,
      });
      // 2. Создаём транзакцию-расход
      await api.transactions.create({
        date: noPhotoForm.date,
        description: noPhotoForm.description || noPhotoForm.name,
        category: noPhotoForm.category,
        amount: -Math.abs(Number(noPhotoForm.amount)),
        status: "Выполнено",
        is_taxable: true,
        is_cashless: noPhotoForm.is_cashless,
        document_id: docRes.document.id,
      });
      // 3. Обновляем список
      setShowNoPhotoModal(false);
      setNoPhotoForm({ name: "", amount: "", date: new Date().toISOString().split("T")[0], category: "Прочее", description: "", is_cashless: false });
      await loadDocs();
    } finally {
  ...
  useEffect(() => {
    ...
    // Загружаем S3-настройки для прямых ссылок на изображения
    api.s3Settings.get().then((res) => {
  ...
  const openCreateTx = () => {
    ...
    setTxForm({ description: desc || "", amount: rawAmount, date: isoDate, category: selected.rec_category || rec?.category || "Прочее", is_cashless: false });
  ...
  const handleCreateTx = async () => {
    ...
    try {
      ...
      if (existingTxId) {
        await api.transactions.update(existingTxId, { date: txForm.date, description: txForm.description, category: txForm.category, amount: -Math.abs(Number(txForm.amount)), status: "Выполнено", is_cashless: txForm.is_cashless });
      } else {
        await api.transactions.create({ date: txForm.date, description: txForm.description, category: txForm.category, amount: -Math.abs(Number(txForm.amount)), status: "Выполнено", is_cashless: txForm.is_cashless });
      }
    ...
    <div className="animate-fade-in flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-2 lg:hidden">
        <button onClick={() => { setPages([]); setShowMultiModal(true); window.scrollTo({ top: 0, behavior: "smooth" }); }}
          className="flex flex-col items-center justify-center gap-1.5 p-3 card-fin border-2 border-dashed border-gold/40 rounded-xl text-gold active:scale-95 transition-transform">
          <Icon name="Camera" size={22} />
          <span className="text-xs font-medium leading-tight text-center">Сфотографировать</span>
        </button>
        <button onClick={() => queueInputRef.current?.click()}
          className="flex flex-col items-center justify-center gap-1.5 p-3 card-fin border-2 border-dashed border-border/60 rounded-xl text-muted-foreground active:scale-95 transition-transform">
          <Icon name="Upload" size={22} />
          <span className="text-xs font-medium leading-tight text-center">Загрузить файл</span>
        </button>
        <button onClick={() => { setShowNoPhotoModal(true); window.scrollTo({ top: 0, behavior: "smooth" }); }}
          className="flex flex-col items-center justify-center gap-1.5 p-3 card-fin border-2 border-dashed border-border/60 rounded-xl text-muted-foreground active:scale-95 transition-transform">
          <Icon name="FileText" size={22} />
          <span className="text-xs font-medium leading-tight text-center">Без фото</span>
        </button>
      ...
      {/* Desktop add without photo button */}
      <div className="hidden lg:block">
        <button onClick={() => { setShowNoPhotoModal(true); window.scrollTo({ top: 0, behavior: "smooth" }); }}
          className="w-full flex items-center justify-center gap-2 p-3 card-fin border-2 border-dashed border-border/60 rounded-xl text-muted-foreground hover:text-foreground hover:border-gold/40 transition-colors active:scale-[0.98]">
          <Icon name="FileText" size={18} />
          <span className="text-sm font-medium">Добавить без фото</span>
        </button>
      </div>
      ...
      <NoPhotoModal
        ...
        form={noPhotoForm}
        saving={noPhotoSaving}
        customCategories={customCategories}
        showNewCat={showNewCat}
        newCatInput={newCatInput}
        onClose={() => setShowNoPhotoModal(false)}
        onSave={handleCreateNoPhoto}
        onFormChange={(patch) => setNoPhotoForm((f) => ({ ...f, ...patch }))}
        onShowNewCat={setShowNewCat}
        onNewCatInput={setNewCatInput}
        onSaveCustomCategory={saveCustomCategory}
        onSetCustomCategories={setCustomCategories}
