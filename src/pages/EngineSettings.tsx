...
import { api, setS3Config, type AiSettings, type S3Settings } from "@/lib/api";

// ─── LocalStorage-кэш ключей (чтобы не терялись при уходе со страницы) ──
const KEYS_CACHE_KEY = "engine_keys_cache";
...
function loadKeysCache(): KeysCache {
  try {
...
function saveKeysCache(cache: KeysCache) {
  try { localStorage.setItem(KEYS_CACHE_KEY, JSON.stringify(cache)); } catch { /* ignore */ }
}
...
export default function EngineSettings() {
  ...
  // S3
  const [s3, setS3] = useState<S3Settings>({ bucket_name: "", endpoint_url: "https://storage.yandexcloud.net", access_key: "", secret_key_masked: "", configured: false });
  ...
  useEffect(() => {
    Promise.all([
      ...
      // Кешируем S3-настройки для прямых ссылок на изображения
      if (s3Res.settings.configured && s3Res.settings.bucket_name) {
        setS3Config({ bucket_name: s3Res.settings.bucket_name, endpoint_url: s3Res.settings.endpoint_url });
      }
      // Восстанавливаем ключи из localStorage-кэша (если бэкенд не вернул)
      const cached = loadKeysCache();
      if (!aiRes.settings.api_key_set && cached.deepseek) setApiKeyInput(cached.deepseek);
      if (!aiRes.settings.proxyapi_key_set && cached.proxyapi) setProxyapiKeyInput(cached.proxyapi);
      if (!aiRes.settings.yandex_key_set && cached.yandex) setYandexKeyInput(cached.yandex);
      if (!aiRes.settings.yandex_folder_set && cached.yandexFolder) setYandexFolderInput(cached.yandexFolder);
      if (!s3Res.settings.secret_key_masked && cached.s3secret) setS3SecretInput(cached.s3secret);
    }).finally(() => setLoading(false));
  }, []);

  // ─── Автосохранение ключей в localStorage при изменении ──────
  useEffect(() => {
    const cache = loadKeysCache();
    if (proxyapiKeyInput) cache.proxyapi = proxyapiKeyInput;
    else delete cache.proxyapi;
    saveKeysCache(cache);
  }, [proxyapiKeyInput]);

  useEffect(() => {
    const cache = loadKeysCache();
    if (yandexKeyInput) cache.yandex = yandexKeyInput;
    else delete cache.yandex;
    saveKeysCache(cache);
  }, [yandexKeyInput]);

  useEffect(() => {
    const cache = loadKeysCache();
    if (yandexFolderInput) cache.yandexFolder = yandexFolderInput;
    else delete cache.yandexFolder;
    saveKeysCache(cache);
  }, [yandexFolderInput]);

  useEffect(() => {
    const cache = loadKeysCache();
    if (s3SecretInput) cache.s3secret = s3SecretInput;
    else delete cache.s3secret;
    saveKeysCache(cache);
  }, [s3SecretInput]);

  // Кэшируем также основной API-ключ (DeepSeek)
  useEffect(() => {
    const cache = loadKeysCache();
    if (apiKeyInput) cache.deepseek = apiKeyInput;
    else delete cache.deepseek;
    saveKeysCache(cache);
  }, [apiKeyInput]);
  ...
  const handleS3Save = async () => {
    ...
    try {
      ...
      // Очищаем кэш S3 ключа — теперь на бэкенде
      const cache = loadKeysCache();
      delete cache.s3secret;
      saveKeysCache(cache);
      // Обновляем кэш S3 для прямых ссылок на изображения
      if (res.settings.configured && res.settings.bucket_name) {
        setS3Config({ bucket_name: res.settings.bucket_name, endpoint_url: res.settings.endpoint_url });
      } else {
        setS3Config(null);
      }
      setS3Saved(true);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Очищаем кэш — ключи теперь на бэкенде
      saveKeysCache({});
      setSaved(true);
  ...
  const handleTest = async () => {
    // Если есть несохранённые ключи — сначала сохраним
    if (apiKeyInput.trim() || proxyapiKeyInput.trim() || yandexKeyInput.trim() || yandexFolderInput.trim()) {
      await handleSave();
    }
    setTesting(true);
    setTestResult(null);
  ...
  return (
    <div className="animate-fade-in w-full max-w-3xl space-y-3 sm:space-y-4">
      ...
      <div className="card-fin p-3 sm:p-4 border border-gold/20">
        ...
        <div className="text-[11px] text-muted-foreground/70 mb-3 pl-3">Две модели работают вместе: одна отвечает в чате, вторая читает фото документов</div>
          ...
          <div className="bg-secondary/50 rounded-lg p-3 flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1.5 flex items-center gap-1.5 flex-wrap">
                ...
                {settings.vision_provider === "yandex" && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${settings.yandex_key_set ? "bg-green-900/30 text-positive" : "bg-red-900/30 text-negative"}`}>
                    {settings.yandex_key_set ? "✓ ключ есть" : "✗ нет ключа"}
                  </span>
                )}
                {settings.vision_provider === "gemini" && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${settings.gemini_key_set ? "bg-green-900/30 text-positive" : "bg-red-900/30 text-negative"}`}>
                    {settings.gemini_key_set ? "✓ ключ есть" : "✗ нет ключа"}
                  </span>
                )}
              </div>
              {/* Быстрое переключение Vision-провайдера */}
              <div className="mt-2 pt-2 border-t border-border/40">
                <div className="text-[10px] text-muted-foreground mb-1.5">Сменить на:</div>
                <div className="flex flex-wrap gap-1">
                  {visionProviders.slice(0, 4).map((vp) => (
                    <button
                      key={vp.id}
                      onClick={() => setSettings((s) => ({ ...s, vision_provider: vp.id }))}
                      className={`text-[10px] px-2 py-0.5 rounded-full border transition-all ${
                        settings.vision_provider === vp.id
                          ? "bg-purple-500/20 border-purple-500/50 text-purple-300"
                          : "border-border/50 text-muted-foreground hover:border-purple-500/30 hover:text-purple-300"
                      }`}
                    >
                      {vp.name.split("(")[0].trim()}
                    </button>
                  ))}
                  <button
                    onClick={() => document.getElementById("vision-select")?.scrollIntoView({ behavior: "smooth", block: "center" })}
                    className="text-[10px] px-2 py-0.5 rounded-full border border-border/50 text-muted-foreground hover:border-gold/40 hover:text-gold transition-all"
      ...
      <div className="card-fin p-3 sm:p-5">
        ...
        <div className="space-y-3">
          ...
          <div id="vision-select" className="rounded-lg border border-purple-900/30 bg-purple-900/10 p-3 sm:p-3.5 space-y-2.5">
            <div className="flex items-start gap-2 flex-wrap">
      ...
      <div className="card-fin p-4 sm:p-5">
        ...
        <div className="mb-4 flex items-center justify-between p-3 rounded-lg border border-border bg-secondary/30">
          <div>
            <div className="text-sm font-medium">{s3.configured ? "Яндекс Object Storage настроен" : "Яндекс Object Storage не настроен"}</div>
            <div className="text-xs text-muted-foreground mt-0.5">{s3.configured ? "Файлы сохраняются в ваш Яндекс бакет" : "Укажите ключи доступа к бакету ниже"}</div>
          </div>
          {s3.configured && (
            <span className="flex items-center gap-1 text-xs text-positive"><Icon name="CheckCircle" size={14} /> Подключено</span>
          )}
        </div>
